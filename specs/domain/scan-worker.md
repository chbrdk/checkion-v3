...
4. Stale reclaim (worker only): `running`/`cancelling` with `updatedAt` older than grace → reset to `queued` (clear session) for retry; web **must not** mark foreign sessions failed when mode=`external`.
