...
describe('domain issue affected pages', () => {
  it('pages through synthetic corpus for a domain issue', () => {
    const first = listIssueAffectedPages('domain-1', 'live-d-iss-1', { page: 1, pageSize: 25 })
    ...
    const last = listIssueAffectedPages('domain-1', 'live-d-iss-1', { page: 125, pageSize: 25 })
  ...
  it('links each page to a resolvable single-page scan id', async () => {
    ...
    const first = listIssueAffectedPages('domain-1', 'live-d-iss-1', { page: 1, pageSize: 5 })!
    for (const item of first.items) {
      expect(item.scanId).toBeTruthy()
      expect(item.scanId.startsWith('dpage__')).toBe(true)
      const overview = getScanOverview(item.scanId)
  ...
  it('sorts fewest issues first when requested', () => {
    const asc = listIssueAffectedPages('domain-1', 'live-d-iss-1', {
      page: 1,
      pageSize: 10,
      sort: 'issues-asc',
    })
  ...
  it('filters by issue-load band', () => {
    const heavy = listIssueAffectedPages('domain-1', 'live-d-iss-1', {
      page: 1,
      pageSize: 50,
      minIssues: 12,
    })
    ...
    const medium = listIssueAffectedPages('domain-1', 'live-d-iss-1', {
      page: 1,
      pageSize: 50,
      minIssues: 5,
      maxIssues: 11,
  ...
  it('prefers seed urls before synthesized paths', () => {
