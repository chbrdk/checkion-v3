...
**Backlinks depth (OpenSEO-parity IA, MSQDX chrome):** Search band (domain · Refresh) → KPIs **DR** · **UR** · Backlinks · Ref. domains (deltas from new/lost) → charts: weekly totals, **new/lost**, TLD, platform types, countries → main **referring-page ledger** (filters All · Dofollow · Nofollow · New · Lost · Gov · Edu) → aside: **Top anchors** · **Referring domains** · **Linked pages** (domain_pages_summary). Capture = full DataForSEO backlinks pack (`specs/domain/seo-dataforseo.md`); persisted on `seo_backlink_snapshots` + `details`.
