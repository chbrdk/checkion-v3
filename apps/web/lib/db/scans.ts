        ...
        persistFailed: async (domainId, error) => {
          ...
          await db
            ...
            .set({
              status: 'failed',
              completedAt: failedAt,
              payload: { scan: { ...queued, status: 'failed', completedAt: failedAt }, error },
              updatedAt: new Date(),
            })
            .where(eq(scans.id, id))
        },
    ...
    try {
      ...
      await db
        ...
        .set({
          status: 'failed',
          completedAt: failedAt,
          payload: { scan: { ...queued, status: 'failed', completedAt: failedAt }, error: message },
          updatedAt: new Date(),
        })
        .where(eq(scans.id, id))
    }
    ...
    },
    {
      insertQueued: async (row) => {
        ...
        await db.insert(domainScans).values({
          ...
          startedAt: row.startedAt,
          completedAt: null,
          payload: { progress: { scanned: 0, total: row.maxPages } },
          updatedAt: new Date(),
          createdAt: new Date(),
        })
      },
