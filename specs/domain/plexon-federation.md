...
|-----------|----------|-------|
| Plexon → CHECKION | `PUT/GET /api/platform/provisioning/projects/{id}` | Upsert / Collection dashboard summary (scans, domain, GEO) via stores. GET returns Waves A–H distillates for METRON suite sync (latest + multi-scan + multi-run domain/GEO lenses/insights; raised caps). `status: archived` hides from default project lists. |
