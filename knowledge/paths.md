...
- Image: root `Dockerfile` fetches sibling `chbrdk/msqdx-ui` at pinned `MSQDX_UI_REF` (`0249eb5270d645e70397731061d39df7ceabf7a3` as of Sep 2026 — includes `LabTile`) into `/workspace/msqdx-ui` (same layout as local `GITHUB/checkion-v3` + `GITHUB/msqdx-ui`). Bump the SHA when barrels need newer primitives — floating `main` clone layers go stale on Coolify and break `lib/msqdx-ui.ts` (e.g. missing `CardActions` / `ChatOverlay` / `LabTile`). Pin must include `ChatOverlay` + `LabTile`; Docker strips DS `node_modules` and symlinks app `node_modules` during build (`msqdx-ui/knowledge/react-types-dedupe.md`).
...
`GET /api/platform/provisioning/projects/{id}` — Plexon / METRON Collection dashboard BFF: catalogs + `latestCompletedScan` + `scoreHistory` + `latestDomainHealth` / `latestGeoDepth` (Wave C/D lenses, presence, insights) for METRON suite sync.
