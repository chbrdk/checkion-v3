...
- Image: root `Dockerfile` fetches sibling `chbrdk/msqdx-ui` at pinned `MSQDX_UI_REF` (`baadd64135bee74b22b46f70a3fb4c1383ddce0c` as of Sep 2026 — includes `SeriesChart` · `LabTile` · `KpiMetric` · `WidgetGrid`) into `/workspace/msqdx-ui` (same layout as local `GITHUB/checkion-v3` + `GITHUB/msqdx-ui`). Bump the SHA when barrels need newer primitives — floating `main` clone layers go stale on Coolify and break `lib/msqdx-ui.ts` (e.g. missing `CardActions` / `ChatOverlay` / `LabTile` / `SeriesChart` / `KpiMetric`). Pin must include `ChatOverlay` + `LabTile` + `SeriesChart` + `KpiMetric`/`WidgetGrid`; Docker strips DS `node_modules` and symlinks app `node_modules` during build (`msqdx-ui/knowledge/react-types-dedupe.md`).
...
`GET /api/platform/provisioning/projects/{id}` — Plexon / METRON Collection dashboard BFF: Waves A–H distillates (latest + multi-scan + multi-run domain/GEO lenses/insights; raised caps) for METRON suite sync.
