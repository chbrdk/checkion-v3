...
|-----------|----------|-------|
| Plexon → CHECKION | `PUT/GET /api/platform/provisioning/projects/{id}` | Upsert / Collection dashboard summary (scans, domain, GEO) via stores. GET returns `latestCompletedScan` + `scoreHistory` + `latestDomainHealth` / `latestGeoDepth` (through Wave E: corpus pages cap, queryRuns, positionMatrix) for METRON suite sync. `status: archived` hides from default project lists. |
