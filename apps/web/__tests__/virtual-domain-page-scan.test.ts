...
describe('virtual domain page scan', () => {
  it('builds a resolvable overview without a seeded scan-single-1 row', () => {
    ...
    expect(overview!.screenshotUrl).toBeUndefined()
    expect(overview!.visualLayers).toBeUndefined()
    expect(overview!.lede).toContain('https://www.provinzial.de/west/')
    expect(overview!.lede.startsWith('checkion:virtual-corpus-lede|')).toBe(true)
    expect(overview!.lede).not.toMatch(/durr/i)
    expect(overview!.seo).toBeUndefined()
  })
})
