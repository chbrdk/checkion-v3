...
describe('platform provisioning projects', () => {
  ...
  it('GET returns real store counts after scans and GEO jobs', async () => {
    ...
    expect(Array.isArray(body.recentScanIssues)).toBe(true)
    expect(Array.isArray(body.recentScanScoreCards)).toBe(true)
    expect(Array.isArray(body.issueRuleFrequency)).toBe(true)
    expect(body.recentScanIssues.length).toBeGreaterThan(0)
    expect(body.recentScanScoreCards.length).toBeGreaterThan(0)
    expect(body.issueRuleFrequency[0]?.ruleId).toBeTruthy()
    expect(Array.isArray(body.scanSeverityByScan)).toBe(true)
    expect(body.scanSeverityByScan.length).toBeGreaterThan(0)
    expect(Array.isArray(body.recentDomainScoreCards)).toBe(true)
    expect(Array.isArray(body.recentDomainSystemic)).toBe(true)
    expect(Array.isArray(body.recentDomainPerformance)).toBe(true)
    expect(Array.isArray(body.recentGeoJobMetrics)).toBe(true)
    expect(Array.isArray(body.recentGeoShareOfVoice)).toBe(true)
    expect(Array.isArray(body.recentGeoPresenceByModel)).toBe(true)
    expect(body.recentGeoJobMetrics.length).toBeGreaterThan(0)
    expect(Array.isArray(body.issueSectionFrequency)).toBe(true)
    expect(Array.isArray(body.recentDomainUx)).toBe(true)
    expect(Array.isArray(body.recentDomainEco)).toBe(true)
    expect(Array.isArray(body.recentDomainSeo)).toBe(true)
    expect(Array.isArray(body.recentDomainCorpusPages)).toBe(true)
    expect(Array.isArray(body.recentGeoRecommendations)).toBe(true)
    expect(Array.isArray(body.recentGeoQueryRuns)).toBe(true)
    expect(Array.isArray(body.recentGeoPromptDuels)).toBe(true)
    expect(Array.isArray(body.recentGeoIntents)).toBe(true)
    expect(Array.isArray(body.recentGeoPresenceByQuery)).toBe(true)
    expect(Array.isArray(body.recentGeoMissVsRival)).toBe(true)
  })
})
