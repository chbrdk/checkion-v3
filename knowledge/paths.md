...
- Modes:
  ...
  - `mode=geo` → `POST /api/geo-jobs` → `/geo/:id/overview` (URL+Project row hidden; `projectId`/`url` resolved silently — auto-creates project when store empty; no `companyId` on GEO body — see `scan-modes.md`)
...
- Create: `POST /api/geo-jobs` · list `GET /api/geo-jobs` · detail `GET /api/geo-jobs/:id` · reading `GET /api/geo-jobs/:id/reading` (`projectId` optional — resolved / auto-created; no `companyId`)
