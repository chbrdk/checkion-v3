...
4. **GEO measurement** (secondary, only when GEO selected) — compact sibling tiles: **Model memory** (`recall`) · **Live search** (`live`). Accessible name via `aria-label` only (`GEO measurement`). Bare `mode=geo` leaves both unselected until the user picks; `measurement=live|recall|both` selects layer(s) and reveals compose.
